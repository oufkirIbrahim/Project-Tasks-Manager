...
export class TasksComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe(({ page = 0, size = 10, filter = 'all' }) => {
  ...
  changePage(page: number): void {
    this.router.navigate([], {
      ...
      queryParams: { page, size: this.pageSize, filter: this.currentFilter },
  ...
  changePageSize(size: number): void {
    this.router.navigate([], {
      relativeTo: this.route,
      queryParams: { page: 0, size, filter: this.currentFilter },
      queryParamsHandling: 'merge'
    });
  }
  ...
  // Helper method to generate array of pages for pagination
  getPageArray(count: number): number[] {
    return Array(count).fill(0).map((_, i) => i);
  }

  // Method to show relevant page numbers around current page
